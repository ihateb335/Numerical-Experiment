...
namespace NE_LW_01_Test_4
{
  class Test_4
  {
    static double alpha, betta, appr_value, true_value, error;
    static int n, m; static string title;

    static void Main(string[] args)
    {
      using (StreamWriter SW = new StreamWriter("Test_4.txt", true))
      {
        CGQ gauss = new CGQ(32);
        Stopwatch clock = new Stopwatch(); clock.Start();

        //title = "DW_856_31"; alpha = 0.7; betta = 0.5; // alpha > betta 
        //appr_value = gauss.Improper(0.0, DW_856_31, 1.0E-13, out double B);
        //true_value = 0.5 * Math.PI / alpha / betta / (alpha + betta);
        //error = Math.Abs(true_value - appr_value);

        //title = "DW_858_711"; n = 3; m = 2; // n > m
        //appr_value = gauss.Improper(0.0, DW_858_711, 1.0E-13, out double B);
        //true_value = 0.5 * Math.PI * m;
        //error = Math.Abs(true_value - appr_value);

        title = "DW_859_005"; m = 2; alpha = 1.7;
        appr_value = gauss.Improper(0.0, DW_859_005, 1.0E-13, out double B);
    ...
    { return 1.0 / (alpha * alpha + x * x) / (betta * betta + x * x); }
